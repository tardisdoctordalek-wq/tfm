...
원본 모드에 있던 "이전 패치 전적을 감쇠 가중치로 블렌딩"은 넣지 못했다. 밸런스 패치를
식별하는 `version` 필드가 **`SoloRankMatch`와 `MatchReplay`에만 있고, 대회 통계 집계에는
없기 때문이다.** 대회 전적을 패치별로 쪼갤 방법이 이 레코드들만으로는 없어서, `prev_weight`
설정은 아예 넣지 않았다. 있는 척하는 설정보다는 없는 게 낫다고 봤다.
...
| `src/stats.rs` | 대회/솔로랭크 전적 집계 (캐시 포함) |
