...
> **두 번 다 이 단계에서, 둘 다 너무 엄격해서 실패했다.**
> v0.1은 후보 키 목록에 단수형 `champion_tier`만 두고 정확 일치로 비교해서 실제 필드인
> 복수형 `champion_tiers`를 한 글자 차이로 놓쳤다.
> v0.2는 이름은 맞췄지만 57개 항목이 **전부** S/A/B/C/D여야 통과시켰다. 그래서 예전에
> 다른 모드를 돌린 내 팀에 섞인 값 하나 때문에 필드 전체가 안 보였다 — 덤프에 찍힌
> 깨끗한 AI 팀들은 정상적으로 읽혔으니, 픽스처 테스트로는 잡힐 수가 없던 버그다.
