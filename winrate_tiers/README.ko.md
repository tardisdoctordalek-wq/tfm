...
확실히 고정하고 싶으면 `tier_path=champion_tiers`.

> 참고로 v0.1이 실패한 이유가 이거였다. 후보 키 목록에 단수형 `champion_tier`만 있었고
> 정확 일치로 비교해서, 실제 필드인 **복수형 `champion_tiers`를 한 글자 차이로 놓쳤다.**
> 지금은 구분자와 복수형을 무시하고 "tier"를 포함하는지로 본다.
